        ...
        for oniom_layer in self.oniom_layers:
            for level in ['ll', 'hl']:
                if self.oniom_layers[oniom_layer][level] is not None:
                    ...
                    if oniom_layer_parameters["max_cluster_num"] > len(
                    ...
                    if oniom_layer_parameters["frozen_core"] not in ["valence", "semicore"]:
                        raise ValueError(
            ...
            for level in ['ll', 'hl']:
                if self.oniom_layers[oniom_layer][level] is not None:
                    ...
                    for basis_set in basis_sets:
                        ...
                        for cluster_num in range(1, oniom_layer_parameters["max_cluster_num"] + 1):
                            if calculation_label not in skzcam_clusters_information_dict[cluster_num]['cluster calculations']:
                                skzcam_clusters_information_dict[cluster_num]['cluster calculations'].append(calculation_label)
                                skzcam_clusters_information_dict[cluster_num]['cluster calculations code kwargs'].append(oniom_layer_parameters['code kwargs'])
