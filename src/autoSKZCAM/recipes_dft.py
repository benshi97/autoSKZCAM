...
def dft_ensemble_flow(
    ...
    dft_ensemble_results = {job_name: {xc_func: None for xc_func in xc_ensemble.keys()} for job_name in ['01-molecule','02-unit_cell','03-slab','04-adsorbate_slab','05-adsorbate_slab_vib','06-molecule_vib','07-slab_vib','08-eint_adsorbate_slab','09-eint_adsorbate','10-eint_slab']}

    # Try to read in completed calculations from the calc_dir
    for xc_func in xc_ensemble.keys():
        for job_name in dft_ensemble_results.keys():
            ...
            if outcar_filename.exists():
                with open(outcar_filename,'r', encoding="ISO-8859-1") as file:
                ...
                dft_ensemble_results[job_name][xc_func] = Summarize(directory=vasp_dir).run(final_atoms,input_atoms)
    ...
    for xc_func in xc_ensemble:
        ...
        if dft_ensemble_results['01-molecule'][xc_func] is None:
            with change_settings(
                ...
                dft_ensemble_results['01-molecule'][xc_func] = relax_job(adsorbate, additional_fields={"job_type": "01-molecule","xc_func": xc_func})
                dft_ensemble_results['01-molecule'][xc_func]['atoms'] = resort_atoms(adsorbate,dft_ensemble_results['01-molecule'][xc_func]['atoms'])
        ...
        if dft_ensemble_results['02-unit_cell'][xc_func] is None:
            with change_settings(
                ...
                dft_ensemble_results['02-unit_cell'][xc_func] = relax_job(unit_cell, additional_fields={"job_type": "02-unit_cell","xc_func": xc_func})
                dft_ensemble_results['02-unit_cell'][xc_func]['atoms'] = resort_atoms(unit_cell,dft_ensemble_results['02-unit_cell'][xc_func]['atoms'])

        # bulk to slab
        if dft_ensemble_results['03-slab'][xc_func] is None:
            initial_slab = slab_gen_func(dft_ensemble_results['02-unit_cell'][xc_func]['atoms'])
            with change_settings(
                {
                ...
                dft_ensemble_results['03-slab'][xc_func] = relax_job(initial_slab, additional_fields={"job_type": "03-slab","xc_func": xc_func})
                dft_ensemble_results['03-slab'][xc_func]['atoms'] = resort_atoms(initial_slab,dft_ensemble_results['03-slab'][xc_func]['atoms'])

        # slab to ads_slab
        if dft_ensemble_results['04-adsorbate_slab'][xc_func] is None:
            initial_adsorbate_slab = adsorbate_slab_gen_func(dft_ensemble_results['01-molecule'][xc_func]['atoms'],dft_ensemble_results['03-slab'][xc_func]['atoms'])
            # sorted_
            with change_settings(
                {
                ...
                dft_ensemble_results['04-adsorbate_slab'][xc_func] = relax_job(initial_adsorbate_slab, additional_fields={"job_type": "04-adsorbate_slab","xc_func": xc_func})
                dft_ensemble_results['04-adsorbate_slab'][xc_func]['atoms'] = resort_atoms(initial_adsorbate_slab,dft_ensemble_results['04-adsorbate_slab'][xc_func]['atoms'])
        ...
        if xc_func in vib_xc_ensemble:
            ...
            if dft_ensemble_results['05-adsorbate_slab_vib'][xc_func] is None:
                with change_settings(
                    {
                        "RESULTS_DIR": Path(calc_dir) / '05-adsorbate_slab_vib' / xc_func,
                        "CREATE_UNIQUE_DIR": False,
                        "GZIP_FILES": True,
                    }
                ):
                    dft_ensemble_results['05-adsorbate_slab_vib'][xc_func] = freq_job(dft_ensemble_results['04-adsorbate_slab'][xc_func]['atoms'], additional_fields={"job_type": "05-adsorbate_slab_vib","xc_func": xc_func})
                    dft_ensemble_results['05-adsorbate_slab_vib'][xc_func]['atoms'] = resort_atoms(dft_ensemble_results['04-adsorbate_slab'][xc_func]['atoms'],dft_ensemble_results['05-adsorbate_slab_vib'][xc_func]['atoms'])
            ...
            if dft_ensemble_results['06-molecule_vib'][xc_func] is None:
                with change_settings(
                    ...
                    dft_ensemble_results['06-molecule_vib'][xc_func] = freq_job(dft_ensemble_results['01-molecule'][xc_func]['atoms'], additional_fields={"job_type": "06-molecule_vib","xc_func": xc_func})
                    dft_ensemble_results['06-molecule_vib'][xc_func]['atoms'] = resort_atoms(dft_ensemble_results['01-molecule'][xc_func]['atoms'],dft_ensemble_results['06-molecule_vib'][xc_func]['atoms'])


            if dft_ensemble_results['07-slab_vib'][xc_func] is None and freeze_surface_vib == False:
                with change_settings(
                    {
                    ...
                    dft_ensemble_results['07-slab_vib'][xc_func] = freq_job(dft_ensemble_results['03-slab'][xc_func]['atoms'], additional_fields={"job_type": "07-slab_vib","xc_func": xc_func})
                    dft_ensemble_results['07-slab_vib'][xc_func]['atoms'] = resort_atoms(dft_ensemble_results['03-slab'][xc_func]['atoms'],dft_ensemble_results['07-slab_vib'][xc_func]['atoms'])


    if geom_error_xc is not None:
        adsorbate_len = len(dft_ensemble_results['01-molecule'][geom_error_xc]['atoms'])
        adsorbate_slab_atoms = dft_ensemble_results['04-adsorbate_slab'][geom_error_xc]['atoms']
        fixed_adsorbate_atoms = dft_ensemble_results['04-adsorbate_slab'][geom_error_xc]['atoms'][:adsorbate_len]
        fixed_slab_atoms = dft_ensemble_results['04-adsorbate_slab'][geom_error_xc]['atoms'][adsorbate_len:]

        for xc_func in xc_ensemble:
            # eint on adsorbate_slab
            if dft_ensemble_results['08-eint_adsorbate_slab'][xc_func] is None:
                dft_ensemble_results['08-eint_adsorbate_slab'][xc_func] = static_job(adsorbate_slab_atoms, additional_fields={"job_type": "08-eint_adsorbate_slab","xc_func": xc_func})
                dft_ensemble_results['08-eint_adsorbate_slab'][xc_func]['atoms'] = resort_atoms(adsorbate_slab_atoms,dft_ensemble_results['08-eint_adsorbate_slab'][xc_func]['atoms'])
            
            # eint on adsorbate
            if dft_ensemble_results['09-eint_adsorbate'][xc_func] is None:
                dft_ensemble_results['09-eint_adsorbate'][xc_func] = static_job(fixed_adsorbate_atoms, additional_fields={"job_type": "09-eint_adsorbate","xc_func": xc_func})
                dft_ensemble_results['09-eint_adsorbate'][xc_func]['atoms'] = resort_atoms(fixed_adsorbate_atoms,dft_ensemble_results['09-eint_adsorbate'][xc_func]['atoms'])

            # eint on slab
            if dft_ensemble_results['10-eint_slab'][xc_func] is None:
                dft_ensemble_results['10-eint_slab'][xc_func] = static_job(fixed_slab_atoms, additional_fields={"job_type": "10-eint_slab","xc_func": xc_func})
                dft_ensemble_results['10-eint_slab'][xc_func]['atoms'] = resort_atoms(fixed_slab_atoms,dft_ensemble_results['10-eint_slab'][xc_func]['atoms'])
