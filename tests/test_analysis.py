...
from autoSKZCAM.analysis import analyze_calculations, compute_skzcam_int_ene, _get_method_int_ene, extrapolate_to_bulk, get_cbs_extrapolation
...
def test_compute_skzcam_int_ene(ref_EmbeddedCluster):
    ...
    ref_skzcam_int_ene = {'Extrapolated Bulk MP2': [-0.16764139187528543, 0], 'Delta_Basis and Delta_Core': [-0.02731886802347076, 0.0061532110649601636], 'FSE Error': [0, 0.002993648282250169], 'DeltaCC': [-0.010131662669145438, 0.0010088038275787748], 'final': [-0.20509192256790162, 0.006916763810504532]}
    ...
    OniomInfo =  {
        "Extrapolated Bulk MP2": {
            ...
            "hl": {
                ...
                "max_cluster_num": 5,
                "code": "orca",
                "element_info": {element: {"basis": "CBS(def2-SVP//def2-TZVPP)"} for element in ["C", "Mg", "O"]},
            },
        },
    }
    ...
    compute_skzcam_int_ene(skzcam_calcs_analysis=skzcam_calcs_analysis, OniomInfo=OniomInfo)
    ...
    skzcam_int_ene = compute_skzcam_int_ene(skzcam_calcs_analysis=skzcam_calcs_analysis, OniomInfo=OniomInfo)
    assert_allclose(skzcam_int_ene['final'],[-0.03847792236925364, 0.01283915060710039], rtol=1e-05, atol=1e-07)
    ...
    with pytest.raises(
        ValueError,
        match="The method cannot be analysed automatically."):
        skzcam_int_ene = compute_skzcam_int_ene(skzcam_calcs_analysis=skzcam_calcs_analysis, OniomInfo=OniomInfo)
    ...
    skzcam_int_ene = compute_skzcam_int_ene(skzcam_calcs_analysis=skzcam_calcs_analysis, OniomInfo=OniomInfo)
    assert skzcam_int_ene['final'][0] == pytest.approx(skzcam_calcs_analysis[5]['orca MP2 valence DZ']['int_ene']['energy'])
...
def test_get_method_int_ene():
    ...
    int_ene_dict = {'energy': 10.0, 'scf_energy': 1.0, 'mp2_corr_energy': 2.0, 'ccsd_corr_energy': 3.0, 'ccsdt_corr_energy': None}
    ...
    with pytest.raises(
        ValueError,
        match="The energy is None."):
        _get_method_int_ene(int_ene_dict, 'ccsdt')

def test_extrapolate_to_bulk():
    
    bulk_int_ene = extrapolate_to_bulk([6, 22, 34, 42, 58, 82, 100],[-0.03284702839164311, -0.08071781613034545, -0.08969258877777975, -0.09523525992926807, -0.10027903776290259, -0.10442318622199309, -0.10807691514310136])
...
def test_get_cbs_extrapolation(ref_EmbeddedCluster):
    ...
    cbs_extrapolated_ene = get_cbs_extrapolation(0.05710643952079408,-0.0899534679109486,0.027170816450961865,-0.12719774638527426,X_size='DZ',Y_size='TZ',family='mixcc')

    assert_allclose((0.017185301080196724, -0.1486152208466108, -0.13142991976641408),cbs_extrapolated_ene, rtol=1e-05, atol=1e-07)

    with pytest.raises(
        ValueError,
        match=re.escape("The cardinal number of Y does not equal X+1")):
        get_cbs_extrapolation(0.05710643952079408,-0.0899534679109486,0.027170816450961865,-0.12719774638527426,family='mixcc',X_size='TZ',Y_size='DZ')
